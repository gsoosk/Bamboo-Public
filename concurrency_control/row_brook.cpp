...
// taking the latch
void Row_brook::lock(txn_man * txn) {
...
// release the latch
void Row_brook::unlock(txn_man * txn) {
        if (likely(g_thread_cnt > 1)) {
            if (unlikely(g_central_man))
...
RC Row_brook::lock_get(lock_t type, txn_man * txn, Access * access) {
  uint64_t *txnids = NULL;
  int txncnt = 0;
  return lock_get(type, txn, txnids, txncnt, access);
}

RC Row_brook::lock_get(lock_t type, txn_man * txn, uint64_t* &txnids, int&txncnt,
    Access * access) {
  assert (CC_ALG == BROOK2PL);
  RC rc;
...
RC Row_brook::lock_release(LockEntry * entry) {
#if PF_CS
  uint64_t starttime = get_sys_clock();
#endif
  lock(entry->txn);
...
bool Row_brook::conflict_lock(lock_t l1, lock_t l2) {
...
inline 
LockEntry * Row_brook::get_entry(Access * access) {
...
inline 
void Row_brook::return_entry(LockEntry * entry) {
  entry->status = LOCK_DROPPED;
  entry->next = NULL;
  entry->prev = NULL;
